...
# Script d'installation EZRAX Central Server v2.0 - Version optimisée
...
update_server() {
    log_message "INFO" "Mise à jour du serveur EZRAX..."
    
    # Vérifier si git est disponible
    if ! command -v git >/dev/null 2>&1; then
        log_message "ERROR" "Git non disponible. Mise à jour manuelle requise."
        return 1
    fi
    ...
    # Ici, vous pourriez ajouter la logique de mise à jour
    # Par exemple, git pull depuis un repository
    
    log_message "WARNING" "Fonctionnalité de mise à jour automatique non implémentée"
    log_message "INFO" "Utilisez le script d'installation pour mettre à jour manuellement"
}
...
show_help() {
    ...
    echo "  update        Mettre à jour le serveur (si disponible)"
