...
# FIXÉ: Imports en haut du fichier
...
class SynFloodScanner(BaseScanner):
    """Détecte les attaques SYN Flood - Version optimisée"""
    ...
    def __init__(self, config, db_manager, ips_manager):
        ...
        # NOUVEAU: Validation des interfaces
    ...
    def setup(self):
        ...
        max_threads = min(len(self.interfaces), 4)  # NOUVEAU: Limite le nb de threads
    ...
    def _start_sniffer(self, interface):
        ...
        """
        try:
            # AMÉLIORÉ: Filtre BPF plus spécifique et sécurisé
        ...
        with self.lock:
            ...
            for src_ip, packets in list(self.syn_counters.items()):
                ...
                if len(recent_packets) >= self.threshold:
                    ...
                    # NOUVEAU: Calcul du taux d'attaque
