...
import logging
import json
import sqlite3  # FIXÉ: Import déplacé en haut
import threading
...
class LogManager:
    """
    Gestionnaire de logs pour l'agent EZRAX IDS/IPS - Version optimisée
    """
    ...
    def __init__(self, config, db_manager):
        ...
        # NOUVEAU: Cache pour les statistiques (évite requêtes répétées)
        self.stats_cache = {
            "data": {},
            "last_update": 0,
    ...
    def get_attack_summary(self, timeframe: int = 86400) -> Dict[str, Any]:
        """
        Génère un résumé des attaques détectées - Version optimisée avec cache
        ...
        try:
            logs = self.db_manager.get_attack_logs(limit=5000, since=since)  # OPTIMISÉ: Limite réduite
            ...
            # OPTIMISÉ: Traitement plus efficace avec dict comprehensions
            attack_types = {}
            attackers = {}
            ...
            # OPTIMISÉ: Top attackers avec tri plus efficace
            top_attackers = [
                {
                    "ip": ip,
                    "count": data["count"],
            ...
            result = {
                ...
                "analysis_time": time.time() - start_time  # NOUVEAU: Métrique de performance
            }
    ...
    def get_blocked_ips_summary(self) -> Dict[str, Any]:
        """
        Génère un résumé des IPs bloquées - Version optimisée
        ...
        try:
            ...
            # OPTIMISÉ: Regroupement plus efficace
            block_reasons = {}
    ...
    def get_agent_stats(self) -> Dict[str, Any]:
        """
        Récupère les statistiques globales de l'agent - Version optimisée
        ...
        try:
            ...
            # OPTIMISÉ: Requêtes combinées pour réduire les accès DB
            ...
            result = {
                ...
                "performance_metrics": self.performance_metrics.copy(),  # NOUVEAU
                "analysis_time": time.time() - start_time
            }
    ...
    def export_logs(self, output_format="json", timeframe=86400, max_records=10000) -> Optional[str]:
        """
        Exporte les logs dans un fichier - Version optimisée
        ...
        try:
            ...
            # OPTIMISÉ: Pagination pour éviter la surcharge mémoire
            all_logs = []
            offset = 0
            batch_size = 1000
